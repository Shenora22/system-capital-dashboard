...
function packageKey(value: string): keyof typeof PACKAGE_NEXT_STEPS | "unknown" {
 ...
 if (normalized === "pro" || normalized.includes("profollowup") || normalized === "149") {
  return "pro";
}
...
function normalizeTallyLead(payload: Record<string, unknown>): NormalizedLead {
  ...
  const rawPackage = firstPresent(fields, ["package", "choose your package", "choose package", "select package", "selected package", "plan", "offer"], "Not provided");
